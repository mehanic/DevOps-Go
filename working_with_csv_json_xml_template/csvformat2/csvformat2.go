...
func (r *Repository) Add(contact map[string]string) {
	...
	// Extract headers from the contact map and sort them
...
func main() {
	contactsFile := filepath.Join(os.TempDir(), "contacts.csv")
	m := NewMain(contactsFile)
	m.Menu()
}

// 2025/01/31 22:48:30 csvformat2.go:24: Initializing contacts repository with file at: /tmp/contacts.csv
// 1) Create Contact
// 2) All Contacts
// 3) Detail of a contact
// 0) Exit
// What do you want to do? ^Csignal: interrupt
