...
func main() {
	// Let's play with variable arrays first using Go's string slicing

	str := "1234567890asdfghjkl"

	fmt.Printf("first character: %s\n", string(str[0]))                 // First character
	fmt.Printf("first three characters: %s\n", str[:3])                 // First three characters
	fmt.Printf("third character onwards: %s\n", str[3:])                // Third character onwards
	fmt.Printf("fourth to sixth character: %s\n", str[3:6])            // Fourth to sixth character
	fmt.Printf("last character: %s\n", string(str[len(str)-1]))        // Last character
